...
class adminUser {
    ...
    public function deleteUser($userId) {
        ...
        // DISCARDED: DATABASE contraints take care of deleting payment-Options from a used when user is deleted. (-> ON DELETE CASCADE option for the foreign key)
    /* public function deleteMultiPaymentMethods($userId){
        // Delete all payment info from  user
        $sql = "DELETE FROM {$this->tbl_payment_items} WHERE userid = ?";
        $stmt = $this->con->prepare($sql);
        $stmt->bind_param("i", $userId);
        $stmt->execute();
        $result = $stmt->affected_rows;
